...
class SemanticSearch:
    ...
    def search(self, query, limit):
        ...
        for i, doc_embed in enumerate(self.embeddings):
            score = cosine_similarity(embedding, doc_embed)
            sim_scores.append((self.document_map[i+1], score))
        return sorted(sim_scores, key=lambda x: x[1], reverse=True)[:limit]
...
def semantic_chunk(
    ...
    while i < n_sentences:
        ...
        if chunks and len(chunk_sentences) <= overlap:
            break
        stripped_chunk = " ".join(chunk_sentences)
        if stripped_chunk == "":
            continue
